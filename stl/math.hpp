...
struct Vec2 {
	...
	Vec2 operator+(const Type& offset) const { return Vec2(x + offset, y + offset); }

	Vec2 operator-(const Vec2& other) const { return Vec2(x - other.x, y - other.y); }

	Vec2 operator-(const Type& offset) const { return Vec2(x - offset, y - offset); }

	Vec2 operator*(const Type& mult) const { return Vec2(x * mult, y * mult); }

	Vec2 operator/(const Type& div) const { return Vec2(x / div, y / div); }
...
struct Rect {
	...
	Rect operator+(const Point& offset) const { return Rect(pos + offset, size); }
	...
	Rect intersection(const Rect& other) const {
		...
		if (rect.size.width <= 0 || rect.size.height <= 0) return Rect(0, 0, 0, 0);
